...
// by: Matthew Fairfax
// http://www.garagegames.com/index.php?sec=mg&
// mod=resource&page=view&qid=506
//
...
void Model_3DS::DrawHelmet()
{
    ...
    for (int i = 0; i < numObjects; i++)
    {
        ...
        // Loop through the faces as
        //sorted by material and draw them
        for (int j = 0; j < Objects[i].numMatFaces; j ++)
        {
            glPushMatrix();

                // Move the model
                glTranslatef(Objects[i].pos.x, Objects[i].pos.y,
                                Objects[i].pos.z);
...
void Model_3DS::CalculateNormals()
{
    ...
    for (int i = 0; i < numObjects; i++)
    {
        for (int g = 0; g < Objects[i].numVerts; g++)
        {
            ...
            length = (float)sqrt((unit.x*unit.x) + (unit.y*unit.y) +
                     (unit.z*unit.z));
...
void Model_3DS::MainChunkProcessor(long length, long findex)
{
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            // This is the mesh information
            //like vertices, faces, and materials
            case EDIT3DS    :
...
void Model_3DS::MaterialChunkProcessor(long length, long findex,
                                       int matindex)
{
    ChunkHeader h;
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            case MAT_NAME   :
                ...
                MaterialNameChunkProcessor(h.len, ftell(bin3ds),
                                           matindex);
                break;
            case MAT_AMBIENT    :
                ...
                break;
            case MAT_DIFFUSE    :
                DiffuseColorChunkProcessor(h.len, ftell(bin3ds),
                                           matindex);
                break;
            case MAT_SPECULAR   :
                //ColorChunkProcessor(h.len, ftell(bin3ds));
            case MAT_TEXMAP :
                // Finds the names of the textures
                // of the material and loads them
                TextureMapChunkProcessor(h.len, ftell(bin3ds),
                                         matindex);
                break;
            default         :
                break;
...
void Model_3DS::MaterialNameChunkProcessor(long length, long findex,
                                           int matindex)
{
    // move the file pointer to the beginning of the main
    // chunk's data findex + the size of the header
...
void Model_3DS::DiffuseColorChunkProcessor(long length, long findex,
                                           int matindex)
{
    ChunkHeader h;
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            case COLOR_RGB  :
                ...
                FloatColorChunkProcessor(h.len, ftell(bin3ds),
                                         matindex);
                break;
            case COLOR_TRU  :
                // A rgb int color chunk
                IntColorChunkProcessor(h.len, ftell(bin3ds),
                                       matindex);
                break;
            case COLOR_RGBG :
...
void Model_3DS::FloatColorChunkProcessor(long length, long findex,
                                         int matindex)
{
    float r;
    float g;
...
void Model_3DS::IntColorChunkProcessor(long length, long findex,
                                       int matindex)
{
    unsigned char r;
    unsigned char g;
...
void Model_3DS::TextureMapChunkProcessor(long length, long findex,
                                         int matindex)
{
    ChunkHeader h;
...
void Model_3DS::MapNameChunkProcessor(long length, long findex,
                                      int matindex)
{
    char name[80];
...
void Model_3DS::ObjectChunkProcessor(long length, long findex,
                                     int objindex)
{
    ChunkHeader h;
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            case TRIG_MESH  :
                ...
                TriangularMeshChunkProcessor(h.len, ftell(bin3ds),
                                             objindex);
                break;
            default         :
                break;
...
void Model_3DS::TriangularMeshChunkProcessor(long length, long findex,
                                             int objindex)
{
    ChunkHeader h;
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            case VERT_LIST  :
                ...
                VertexListChunkProcessor(h.len, ftell(bin3ds),
                                         objindex);
                break;
            case LOCAL_COORDS   :
    ...
    while (ftell(bin3ds) < (findex + length - 6))
    {
        ...
        switch (h.id)
        {
            case FACE_DESC  :
                ...
                FacesDescriptionChunkProcessor(h.len, ftell(bin3ds),
                                               objindex);
                break;
            default         :
                break;
...
void Model_3DS::VertexListChunkProcessor(long length, long findex,
                                         int objindex)
{
    ...
    // Read the vertices, switching the y and z
    // coordinates and changing the sign of the z coordinate
    for (int i = 0; i < numVerts * 3; i+=3)
    {
        fread(&Objects[objindex].Vertexes[i],
              sizeof(GLfloat),1,bin3ds);
        fread(&Objects[objindex].Vertexes[i+2],
              sizeof(GLfloat),1,bin3ds);
        fread(&Objects[objindex].Vertexes[i+1],
              sizeof(GLfloat),1,bin3ds);

        // Change the sign of the z coordinate
        Objects[objindex].Vertexes[i+2] =
                                      -Objects[objindex].Vertexes[i+2];
    }
...
void Model_3DS::TexCoordsChunkProcessor(long length, long findex,
                                        int objindex)
{
    ...
    {
        fread(&Objects[objindex].TexCoords[i],
              sizeof(GLfloat),1,bin3ds);
        fread(&Objects[objindex].TexCoords[i+1],
              sizeof(GLfloat),1,bin3ds);
    }
...
void Model_3DS::FacesDescriptionChunkProcessor(long length,long findex,
                                               int objindex)
{
    ChunkHeader h;
    ...
    if (numMatFaces > 0)
    {
        ...
        // lists of faces divided by material
        ...
        while (ftell(bin3ds) < (findex + length - 6))
        {
            ...
            switch (h.id)
            {
                case FACE_MAT   :
                    ...
                    FacesMaterialsListChunkProcessor(h.len,
                                           ftell(bin3ds), objindex, j);
                    j++;
                    break;
                default         :
                    break;
                                                 ...
                                                 int subfacesindex)
{
    ...
    // Allocate an array to hold the list of
    // faces associated with this material
    Objects[objindex].MatFaces[subfacesindex].subFaces =
                                          new GLushort[numEntries * 3];
    // Store this number for later use
    Objects[objindex].MatFaces[subfacesindex].numSubFaces =
                                                        numEntries * 3;
    ...
    for (int i = 0; i < numEntries * 3; i+=3)
    {
        ...
        Objects[objindex].MatFaces[subfacesindex].subFaces[i] =
                                     Objects[objindex].Faces[Face * 3];
        Objects[objindex].MatFaces[subfacesindex].subFaces[i+1] =
                                  Objects[objindex].Faces[Face * 3 + 1];
        Objects[objindex].MatFaces[subfacesindex].subFaces[i+2] =
                                  Objects[objindex].Faces[Face * 3 + 2];
    }
